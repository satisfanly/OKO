...
# Backported recipe from meta-openembedded 
...
FILES:${PN} += "\
    ${systemd_system_unitdir}/kmsconvt@.service \
    ${datadir}/terminfo \
"
...
do_install:append() {
    ...
    if ${@bb.utils.contains('PACKAGECONFIG', 'opengl', 'true', 'false', d)}; then
        mv ${D}${sysconfdir}/kmscon/kmscon.conf.example \
           ${D}${sysconfdir}/kmscon/kmscon.conf
        sed -e "s@#drm@drm@g" \
